...
def subdominios_view(request):
    ...
    try:
        ...
        messages.error(request, f"Error al cargar tu configuración: {e}")
    ...
    if request.method == "POST":
        ...
        if not re.match(r'^[a-z0-9\-]{3,30}$', subdomain):
            messages.error(request, "El subdominio debe tener entre 3 y 30 caracteres alfanuméricos o guiones.")
        ...
        if action == "add":
            ...
            try:
                ...
                if ok:
                    ...
                    messages.error(request, f"Subdominio guardado, pero {msg}")

            except Exception as e:
                messages.error(request, f"Error al crear el subdominio: {e}")
                logger.exception(f"Error añadiendo subdominio '{full_domain}' para '{request.user.username}'")
            ...
            if len(user_routes) < original_count:
                try:
                    user_cfg_obj.save()
                    ok, msg = construir_configuracion_global(iniciado_por=f"Delete subdomain by {request.user.username}")
                    if ok:
                        messages.success(request, f"Subdominio '{full_domain}' eliminado. {msg}")
                    else:
                        messages.error(request, f"Subdominio eliminado de la DB, pero {msg}")
                except Exception as e:
                    messages.error(request, f"Error al eliminar subdominio: {e}")
                    logger.exception(f"Error al eliminar subdominio '{full_domain}' para '{request.user.username}'")
            else:
                messages.warning(request, f"No se encontró el subdominio '{full_domain}'.")

        return redirect("subdominios_view")

    return render(request, "subdominios.html", {
